...
export function Window({ title, children, className = "" }: WindowProps) {
  ...
  return (
    <motion.div
      ...
      className={`fixed inset-y-4 left-1/2 -translate-x-1/2 w-[80vw] max-w-[80vw] z-40 flex flex-col bg-card/95 backdrop-blur-md border border-border rounded-xl shadow-2xl ${className}`}
    >
      {/* Window chrome */}
